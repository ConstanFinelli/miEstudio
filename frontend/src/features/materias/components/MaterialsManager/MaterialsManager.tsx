      ...
      <div className={styles.materialLeft}>
        ...
        <div className={styles.materialMetaContent}>
          <div className={styles.materialTitleRow}>
            ...
            <span
              className={`${styles.catBadge} ${getCategoryBadgeClass(mat.categoria)}`}
            >
              {getCategoryLabel(mat.categoria)}
            </span>
            {viewMode === "CATEGORIAS" && mat.unidad && (
              <span className={styles.unitTagBadge}>{mat.unidad}</span>
            )}
          </div>
          <div className={styles.materialMeta}>
            ...
            {mat.cantPaginas ? (
              <span>
                {" "}
                · {mat.cantPaginas} {mat.cantPaginas === 1 ? "pág" : "págs"}
              </span>
            ) : null}{" "}
              ...
              <button
                ...
                className={`${styles.materialFilterChip} ${
                  selectedCategory === tab.id
                    ? styles.materialFilterChipActive
                    : ""
                }`}
      ...
      {filteredMaterials.length > 0 ? (
          ...
          <div className={styles.unitsContainer}>
            {unitGroups.map((group) => {
              ...
              const totalBytes = group.items.reduce(
                (acc, curr) => acc + (curr.tamanioBytes || 0),
                0,
              );
                ...
                <div key={group.unitName} className={styles.unitGroupCard}>
                  ...
                  >
                    <div className={styles.unitHeaderLeft}>
                      ...
                      <span className={styles.unitGroupTitle}>
                        {group.unitName}
                      </span>
                      <span className={styles.unitCountBadge}>
                        {group.items.length}{" "}
                        {group.items.length === 1 ? "archivo" : "archivos"}
                      </span>
        ...
        <div className={styles.materialsEmptyState}>
          ...
          <p className={styles.emptyStateDesc}>
            Subí diapositivas, guías prácticas o parciales viejos en PDF
            organizados por unidades para tenerlos siempre a mano.
          </p>
          <button
