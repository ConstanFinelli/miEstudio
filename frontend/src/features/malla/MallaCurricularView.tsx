    ...
    <div className={styles.mallaContainer}>
      ...
      <div className={styles.mallaHeader}>
        <div className={styles.headerTop}>
          <div className={styles.titleGroup}>
            ...
            <div>
              <h1 className={styles.mainTitle}>Malla Curricular & Correlatividades</h1>
              <p className={styles.subtitle}>
                {activeCarrera?.nombre || 'Carrera Universitaria'} • {activeCarrera?.facultad_sede || 'Plan de Estudio'}
              </p>
            </div>
          </div>
          ...
          <div className={styles.statsBar}>
            <div className={styles.statChip}>
              ...
              <span className={styles.statChipVal} style={{ color: 'var(--emerald)' }}>
            ...
            <div className={styles.statChip}>
              <span>Cursando:</span>
              <span className={styles.statChipVal} style={{ color: 'var(--primary-glow)' }}>
                {stats.cursandoCount}
              </span>
            </div>
            <div className={styles.statChip}>
            ...
            <div className={styles.statChip}>
              ...
              <span className={styles.statChipVal} style={{ color: 'var(--text-muted)' }}>
        ...
        <div className={styles.controlsBar}>
          <div className={styles.controlsLeft}>
            ...
            <div className={styles.filterButtonGroup}>
              <button
                type="button"
                className={`${styles.filterBtn} ${filterMode === 'TODAS' ? styles.filterBtnActive : ''}`}
                onClick={() => setFilterMode('TODAS')}
                title="Mostrar conexiones de Cursar y Rendir"
              >
                Todas las conexiones
              </button>
              <button
                type="button"
                className={`${styles.filterBtn} ${filterMode === 'SOLO_CURSAR' ? styles.filterBtnActive : ''}`}
                onClick={() => setFilterMode('SOLO_CURSAR')}
                title="Mostrar solo correlativas requeridas para cursar"
              >
                Para Cursar
              </button>
              <button
                type="button"
                className={`${styles.filterBtn} ${filterMode === 'SOLO_RENDIR' ? styles.filterBtnActive : ''}`}
                onClick={() => setFilterMode('SOLO_RENDIR')}
                title="Mostrar solo correlativas requeridas para examen final"
              >
                Para Rendir
              </button>
            </div>
            ...
            >
              ...
              <option value="HABILITADAS">Solo Habilitadas para cursar</option>
              <option value="CURSANDO">Solo Cursando</option>
              <option value="REGULARES">Solo Regulares (para Final)</option>
              <option value="APROBADAS">Solo Aprobadas / Promocionadas</option>
              <option value="BLOQUEADAS">Solo Bloqueadas</option>
            </select>
          </div>
          ...
          <div className={styles.controlsRight}>
            ...
            >
              ...
              <span>{isSimulationActive ? 'Modo Simulación Activo' : 'Simular Aprobaciones'}</span>
            </button>
          </div>
        </div>
      </div>
      ...
      {isSimulationActive && (
        <div className={styles.simBanner}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <span>
              <strong>Modo Simulación Activo:</strong> Haz clic en el botón <em>"Simular"</em> de cualquier materia pendiente para proyectar en tiempo real qué asignaturas de años superiores desbloquearías.
              {simulatedApprovedIds.size > 0 && (
                <span style={{ marginLeft: '6px', color: 'var(--emerald)' }}>
                  ({simulatedApprovedIds.size} {simulatedApprovedIds.size === 1 ? 'materia simulada' : 'materias simuladas'})
                </span>
              )}
            </span>
          </div>
      ...
      {materias.length === 0 ? (
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '12px', color: 'var(--text-muted)' }}>
          ...
          <p style={{ fontSize: '14px' }}>No hay materias registradas en la carrera activa.</p>
        </div>
      ) : (
        <MallaCanvas
          yearsList={yearsList}
