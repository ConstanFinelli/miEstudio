...
const renderInlineHtml = (text: string): string => {
  ...
  segments.forEach(seg => {
    ...
    if (seg.startsWith('$') && seg.endsWith('$') && seg.length > 2) {
      ...
      parts.push(`<code class="inline-code">${escapeHtml(seg.slice(1, -1))}</code>`);
  ...
  while (i < lines.length) {
    ...
    if (line.trim().startsWith('$$')) {
      ...
      try {
        ...
        htmlParts.push(`<div class="math-container"><pre>${escapeHtml(formula)}</pre></div>`);
      }
      continue;
    }
...
export const generateNotePdfBlob = async (note: ApunteNota): Promise<Blob> => {
  ...
  container.innerHTML = `
    <div style="border-bottom: 2px solid #e5e7eb; padding-bottom: 12px; margin-bottom: 18px;">
      <div style="font-size: 11px; font-weight: bold; color: #4f46e5; text-transform: uppercase;">
        miEstudio · Cuaderno Académico — ${escapeHtml(note.materiaNombre || 'Materia')}
      </div>
      <h1 style="font-size: 24px; font-weight: bold; margin: 8px 0 6px; color: #111827;">${escapeHtml(
        note.titulo
      )}</h1>
      <div style="font-size: 11px; color: #6b7280; display: flex; gap: 12px;">
        <span>Materia: <strong>${escapeHtml(note.materiaNombre || 'General')}</strong></span>
        ${
          note.carpeta
            ? `<span>Unidad: <strong>${escapeHtml(note.carpeta)}</strong></span>`
            : ''
        }
        <span>Fecha: <strong>${
    ...
    <div style="font-size: 13px; line-height: 1.6;">
      ${renderMarkdownBodyHtml(note.contenidoMarkdown || '')}
    </div>
  `;
 ...
 */
export const exportSingleNoteDirectPdf = async (note: ApunteNota): Promise<void> => {
